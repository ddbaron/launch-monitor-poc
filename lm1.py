...
output_filename = f'{filename}_lm1_{width}x{height}_output.mp4'
...
while True:
    ...
    # Check if the buffer size has reached the desired frames_before_exit
    if len(frame_buffer) >= frames_before_exit:
        break

# Write frames from the buffer to the output video
